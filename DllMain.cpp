...
HRESULT __stdcall hkPresent(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags, HMODULE hMod)
{
	...
	if (openmenu) {
		...
		if (tabb == 0) {
			...
			ImGui::Checkbox(xorstr(u8"×åëîâåê Ïàóê"), &Vars::Misc::spiderman);
			ImGui::Checkbox(xorstr(u8"Ôåéêîâûå Ëàãè"), &Vars::Misc::fakelag);
			ImGui::Checkbox(xorstr(u8"Íåò Îòäà÷è Îðóæèÿ"), &Vars::Misc::norecoil);
		}
		else if (tabb == 3) {
			ImGui::Text(xorstr(u8"Ðàçðàáîò÷èê ìåíþ: Darwin"));
			ImGui::Text(xorstr(u8"Êíîïêà ìåíþ: Home | Âûêëþ÷èòü âñå: End"));
		}
		// Òàáëèöû ×åêÁîêñû
